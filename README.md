...
- **Защищенный вебук зачислений**: Проверка SHA-256 хеша в поле `signature` тела JSON для безопасной интеграции с платежным провайдером.
...
sequenceDiagram
    ...
    Provider->>API: POST /api/v1/webhooks/payment (JSON payload + signature)
    API->>API: Проверка SHA-256 хеша конкатенации полей и секрета
...
- **Безопасный Docker-контейнер**: Сборка и запуск приложения под непривилегированным пользователем (non-root) на базе Debian (`python:3.12-slim`).
